-- Relacionamento N:N entre api_services e sistema_destino_config
CREATE TABLE IF NOT EXISTS api_service_destinos (
  api_service_id UUID NOT NULL REFERENCES api_services(id) ON DELETE CASCADE,
  sistema_destino_config_id INTEGER NOT NULL REFERENCES sistema_destino_config(id) ON DELETE CASCADE,
  criado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (api_service_id, sistema_destino_config_id)
);

CREATE INDEX IF NOT EXISTS idx_api_service_destinos_api_service_id
  ON api_service_destinos(api_service_id);

CREATE INDEX IF NOT EXISTS idx_api_service_destinos_sistema_destino_config_id
  ON api_service_destinos(sistema_destino_config_id);

-- Permite rastrear qual Api Service originou o raw, para decidir destinos pela configuração do service
ALTER TABLE operacoes_raw
  ADD COLUMN IF NOT EXISTS source_service_id UUID NULL REFERENCES api_services(id);

CREATE INDEX IF NOT EXISTS idx_operacoes_raw_source_service_id
  ON operacoes_raw(source_service_id);

-- Atualiza a normalização para usar api_service_destinos quando source_service_id estiver preenchido e configurado
...
DECLARE
  ...
  v_fin_hash TEXT;
BEGIN
  SELECT payload, source_service_id
  INTO v_payload, v_source_service_id
  FROM operacoes_raw
  ...
  IF v_source_service_id IS NOT NULL
     AND EXISTS (SELECT 1 FROM api_service_destinos WHERE api_service_id = v_source_service_id)
  THEN
    FOR v_destino IN
      SELECT sdc.nome, sdc.endpoint_url, sdc.metodo, sdc.entidade_view
      FROM sistema_destino_config sdc
      JOIN api_service_destinos asd ON asd.sistema_destino_config_id = sdc.id
      WHERE asd.api_service_id = v_source_service_id
        AND sdc.ativo = true
    LOOP
      ...
      ON CONFLICT (sistema_nome, entidade_view, payload_id)
      DO UPDATE SET
        endpoint_url = EXCLUDED.endpoint_url,
        metodo = EXCLUDED.metodo,
        entidade = EXCLUDED.entidade,
        status = 'pendente',
        tentativas = CASE WHEN integracao_pendente.status = 'pendente' THEN integracao_pendente.tentativas ELSE 0 END,
