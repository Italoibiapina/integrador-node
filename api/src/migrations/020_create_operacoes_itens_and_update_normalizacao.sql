-- 1) Tabela de itens da operação (histórico por hash)
CREATE TABLE IF NOT EXISTS operacoes_itens (
  id BIGSERIAL PRIMARY KEY,
  operacao_id BIGINT NOT NULL REFERENCES operacoes(id) ON DELETE CASCADE,
  id_venda_externo TEXT NOT NULL,
  id_item_externo TEXT NOT NULL,
  cor TEXT NULL,
  produto TEXT NULL,
  tamanho TEXT NULL,
  quantidade NUMERIC(14,4) NULL,
  valor_unitario NUMERIC(14,2) NULL,
  preco_promocional BOOLEAN NULL,
  valor_desconto_item NUMERIC(14,2) NULL,
  valor_venda_original NUMERIC(14,2) NULL,
  campanha_promocional JSONB NULL,
  valor_item_com_desconto NUMERIC(14,2) NULL,
...
CREATE INDEX IF NOT EXISTS idx_operacoes_itens_operacao_id
  ON operacoes_itens(operacao_id);

CREATE INDEX IF NOT EXISTS idx_operacoes_itens_id_venda_externo
  ON operacoes_itens(id_venda_externo);

CREATE INDEX IF NOT EXISTS idx_operacoes_itens_id_item_externo
  ON operacoes_itens(id_item_externo);

CREATE INDEX IF NOT EXISTS idx_operacoes_itens_data_extracao
  ON operacoes_itens(data_extracao DESC, id DESC);

-- 2) Procedure de normalização incluindo carga de operacoes_itens com deduplicação por hash do último item id
...
BEGIN
  ...
  -- Histórico de itens (insert-only) comparando hash do último registro do mesmo id_item_externo
  FOR v_item IN
    SELECT value
  ...
  LOOP
    ...
    VALUES (
      ...
      v_item_tamanho,
      v_item_quantidade,
      v_item_valor_unitario,
      v_item_preco_promocional,
      v_item_valor_desconto,
      v_item_valor_original,
      v_item_campanha,
      v_item_valor_com_desconto,
      v_item_info_complementares,
      v_item,
      v_item_hash
    );
  END LOOP;
