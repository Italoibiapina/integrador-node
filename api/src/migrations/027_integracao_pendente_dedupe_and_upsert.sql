-- Garante idempotência da outbox: apenas 1 registro por (sistema_nome, entidade_view, payload_id)

WITH ranked AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY sistema_nome, entidade_view, payload_id
      ORDER BY id DESC
    ) AS rn
  FROM integracao_pendente
)
DELETE FROM integracao_pendente ip
USING ranked r
WHERE ip.id = r.id
  AND r.rn > 1;

CREATE UNIQUE INDEX IF NOT EXISTS ux_integracao_pendente_sistema_entidade_view_payload
  ON integracao_pendente (sistema_nome, entidade_view, payload_id);

-- Atualiza a normalização para fazer upsert em integracao_pendente (mantém 1 pendência representando o estado mais recente)
