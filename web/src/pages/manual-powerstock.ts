export type ManualPowerStockPageDeps = {
...
export function renderManualPowerStockPage(deps: ManualPowerStockPageDeps): HTMLElement {
  ...
  root.innerHTML = `
    <h1>Buscar Operações no PowerStock</h1>
    <p class="muted">Disparo manual do serviço de captura de operações (com ou sem período).</p>
    ...
    <div class="grid">
      <div class="card">
        ...
        <div class="actions" style="margin-top: 12px;">
          <button id="btnRunDefault">Buscar Dados de Ontem</button>
          <button id="btnRunWithDates">Buscar dados pelas datas informadas</button>
        </div>
  ...
  function renderHint() {
    ...
    hintEl.textContent = name
      ? `Serviço: ${name}`
      : 'Serviço PowerStock não encontrado no cadastro. Verifique se existe um service "PowerStock Buscar Operações".';
  }
  ...
  function renderHistory() {
    historyRowsEl.innerHTML = history.map((b) => `
      <tr data-id="${b.id}">
        ...
        <td>${formatDateOnly(extractRange(b).start)}</td>
        <td>${formatDateOnly(extractRange(b).end)}</td>
  ...
  async function loadServices() {
    ...
    const preferred =
      sorted.find((s) => s.name === 'PowerStock Buscar Operações') ??
      sorted.find((s) => String(s.service_name || '') === 'PowerStockGetOperationsService') ??
      sorted.find((s) => s.name.toLowerCase().includes('powerstock') && s.name.toLowerCase().includes('opera')) ??
      null;
  ...
  async function runExecute(payload?: { dataEmissaoInicio?: string; dataEmissaoFim?: string }) {
    if (!selectedService?.id) {
      alert('Serviço PowerStock não encontrado no cadastro.');
      return;
    }
    outEl.textContent = '';
    setBusy(true);
